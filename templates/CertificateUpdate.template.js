export const certificateUpdateTemplate = (userName, subject, status) => {
  const statusColor = 
    ...
    status?.toLowerCase() === "rejected" ? "#EF4444" : "#6366F1";
  ...
  return {
    subject: `📜 Status Update: Certificate Request for "${subject}" has been ${status.toUpperCase()}`,
    text: `Hello ${userName}, your certificate request for "${subject}" has been ${status}. Please visit your dashboard for details.`,
    html: `
      <div style="background-color: #0B0F19; padding: 40px 20px; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #E2E8F0; text-align: center;">
        <div style="max-width: 500px; margin: 0 auto; background-color: #111827; border: 1px solid #1E293B; border-radius: 24px; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5);">
          <div style="background-color: #0F172A; padding: 20px; border-bottom: 1px solid #1E293B;">
            <p style="margin: 0; font-size: 11px; color: #6366F1; font-weight: 800; text-transform: uppercase; letter-spacing: 0.15em;">Certificate Authorization</p>
          </div>
          <div style="padding: 40px 30px;">
            ...
            <h2 style="margin: 0 0 16px; font-size: 20px; color: #F8FAFC; font-weight: 700;">Final Review Complete</h2>
            <p style="margin: 0 0 24px; font-size: 14px; line-height: 1.6; color: #94A3B8; text-align: left;">
              Hello <strong>${userName}</strong>, <br/><br/>
              The administrative review for your certificate request <strong>"${subject}"</strong> is complete.
            </p>
          ...
          <div style="background-color: #0F172A; padding: 16px; border-top: 1px solid #1E293B;">
            <p style="margin: 0; font-size: 11px; color: #475569; text-transform: uppercase; letter-spacing: 0.1em;">Official Institutional Portal</p>
          </div>
        </div>
      </div>
    `,
